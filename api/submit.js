export default async function handler(req, res) {
  ...
  console.log('DEBUG: GOOGLE_SCRIPT_URL exists:', !!GOOGLE_SCRIPT_URL);
  console.log('DEBUG: GOOGLE_SCRIPT_URL length:', GOOGLE_SCRIPT_URL ? GOOGLE_SCRIPT_URL.length : 0);
  ...
  try {
    ...
    if (GOOGLE_SCRIPT_URL) {
      console.log('DEBUG: Sending to Google Script...');
      promises.push(
        (async () => {
          try {
            const scriptBody = JSON.stringify(data);
            console.log('DEBUG: Initial POST to Google Script');
            ...
            console.log('DEBUG: Initial response status:', resp.status);
            if (resp.status >= 300 && resp.status < 400) {
              const loc = resp.headers.get('location');
              console.log('DEBUG: Redirect location exists:', !!loc);
              if (loc) {
                ...
                console.log('DEBUG: Redirect response status:', resp.status);
              }
            }
            return resp;
          } catch (err) {
      ...
      console.log('DEBUG: GOOGLE_SCRIPT_URL is empty/falsy, skipping webhook');
    }
